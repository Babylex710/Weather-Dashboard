...
var APIKey = "5b4e66a593321390e6ab22d077b188f4";
...
function getWeatherData(search) {
  ...
  var weatherGeoURL = `https://api.openweathermap.org/geo/1.0/direct?q=${search}&appid=${APIKey}`;
  ...
  fetch(weatherGeoURL)
    ...
    .then(function (data) {
      ...
      //API for weather
      fetch(
        `https://api.openweathermap.org/data/2.5/onecall?lat=${data[0].lat}&lon=${data[0].lon}&units=imperial&exclude=minutely,hourly&appid=${APIKey}`
      )
        .then(function (response) {
          // console.log(response);
          return response.json();
...
search.addEventListener("click", console.log(search));
